...
class BotHandler:
    def __init__(self, db_client: BotDB):
        self.db = db_client
    ...
    def handle_message(self, message):
        ...
        logger.info(f"Received message from chat {chat_id} (user: {username or first_name}): '{text}'")

        # Ensure chat is registered
        if not self.db.check_if_chat_exists(chat_id):
            try:
                self.db.insert_chat(chat_id, username, first_name)
                logger.info(f"Chat {chat_id} registered successfully.")
            except Exception as e:
                _, _, exc_tb = sys.exc_info()
                logger.error(f"Failed to register chat {chat_id} on line {exc_tb.tb_lineno}: {e}", exc_info=True)
        ...
        if text == "/start":
            ...
            logger.warning(f"Unrecognized command from chat {chat_id}: '{text}'")
    ...
    def _register_class_command(self, chat_id, text):
        logger.info(f"Handling /register_class command for chat {chat_id}.")
        text = text.replace("/register_class", "")
        parts = text.split("|")
        if len(parts) < 3:
            logger.warning(f"Invalid /register_class usage by chat {chat_id}: '{text}'")
        ...
        logger.info(f"Parsed /register_class command for chat {chat_id}: {class_id}, {name}, {semester}")

        try:
            self.db.insert_class(class_id, name, semester)
            logger.info(f"Class '{name}' ({class_id}) registered by chat {chat_id}.")
            return f"Disciplina '{name}' ({class_id}) registrada com sucesso!"
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            logger.error(f"Error registering class {class_id} for chat {chat_id} on line {exc_tb.tb_lineno}: {e}", exc_info=True)
            return f"Erro ao registrar disciplina: {e}"

    def _add_absence_command(self, chat_id, text):
        logger.info(f"Handling /add_absence command for chat {chat_id}.")
        parts = text.split(maxsplit=1)
        if len(parts) < 2:
            logger.warning(f"Invalid /add_absence usage by chat {chat_id}: '{text}'")
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Absence added for chat {chat_id} in class {class_id}. New count: {count}.")
                return f"Falta adicionada para '{class_id}'. Total de faltas: {count}."
            else:
                logger.info(f"Attempted to add absence for non-existent class '{class_id}' by chat {chat_id}.")
                return f"Erro: Disciplina '{class_id}' não encontrada. Registre-a primeiro com /register_class."
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            logger.error(f"Error adding absence for chat {chat_id}, class {class_id} on line {exc_tb.tb_lineno}: {e}", exc_info=True)
            return f"Erro ao adicionar falta: {e}"

    def _my_absences_command(self, chat_id, text):
        logger.info(f"Handling /my_absences command for chat {chat_id}.")
        parts = text.split(maxsplit=1)
        if len(parts) < 2:
            logger.warning(f"Invalid /my_absences usage by chat {chat_id}: '{text}'")
        ...
        try:
            count = self.db.get_absence_count(chat_id, class_id)
            if count > 0:
                logger.info(f"Retrieved {count} absences for chat {chat_id} in class {class_id}.")
                return f"Você tem {count} falta(s) em '{class_id}'."
            else:
                logger.info(f"No absences found for chat {chat_id} in class {class_id}.")
                return f"Você não tem faltas registradas em '{class_id}' ou a disciplina não existe."
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            logger.error(f"Error getting absences for chat {chat_id}, class {class_id} on line {exc_tb.tb_lineno}: {e}", exc_info=True)
            return f"Erro ao buscar faltas: {e}"

    def _total_absences_command(self, chat_id):
        logger.info(f"Handling /total_absences command for chat {chat_id}.")
        try:
            total_count = self.db.get_total_absences(chat_id)
            logger.info(f"Retrieved total {total_count} absences for chat {chat_id}.")
    ...
    def _list_classes_command(self, chat_id):
        logger.info(f"Handling /list_classes command for chat {chat_id}.")
        try:
            classes = self.db.get_all_classes(chat_id)
            if not classes:
                logger.info(f"No classes registered for chat {chat_id}.")
                return "Nenhuma disciplina registrada ainda. Use /register_class para adicionar uma."
            
            response = "Disciplinas registradas:\n"
            for cls in classes:
                class_id, name, semester = cls
                response += f"- {name} ({class_id})"
                if semester:
                    response += f" - Semestre: {semester}"
                response += "\n"
            logger.info(f"Listed {len(classes)} classes for chat {chat_id}.")
            return response
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            logger.error(f"Error listing classes for chat {chat_id} on line {exc_tb.tb_lineno}: {e}", exc_info=True)
